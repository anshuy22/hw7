  ...
  firebase.auth().onAuthStateChanged(async function(user) {
    ...
    if (user) {
      ...
      let apiKey = '31d3aa3e7d70dc0219a4a979955702e7'
    ...
    for (let i=0; i<movies.length; i++) {
      ...
      if(document.querySelector(`.movie-${movie.id}-${user.uid}`).classList.contains('opacity-20')){
        event.preventDefault()
        await db.collection('watched').doc(`${movie.id}-${user.uid}`).delete()
        document.querySelector(`.movie-${movie.id}-${user.uid}`).classList.remove('opacity-20') 
        
        } else{
        
        document.querySelector(`.movie-${movie.id}-${user.uid}`).classList.add('opacity-20')
        ...
        <button class="text-green-500 text-align:left">Signed in as ${user.displayName}</button>
        <button class="text-pink-500 text-align:right underline sign-out"> Sign Out</button>`
